...
/**
 * English language strings for local_mondaysync.
 *
...
$string['fieldpicker_standard'] = 'Standard';
$string['fieldpicker_advanced'] = 'Advanced';
$string['orphanedgroupheader'] = 'No longer available';
$string['enabledfields'] = 'Fields available for mapping';
$string['enabledfields_desc'] = 'Which Standard, Advanced, and custom profile fields the mapping wizard\'s "sync to" dropdown actually offers, grouped the same way they appear there. Unticking a field that\'s already in use by a saved mapping doesn\'t delete that mapping - it shows up as "No longer available" in the wizard next time it\'s opened, exactly like it already does when a custom profile field is deleted entirely, and stops being synced until it\'s either re-enabled here or removed from that mapping. New custom profile fields created after this setting is first saved are NOT automatically enabled - they need to be ticked here before they appear in the dropdown.';
...
$string['advancedfield_timecreated'] = 'Account created (timecreated)';
$string['advancedfield_username'] = 'Username (username)';
$string['advancedfield_confirmed'] = 'Account confirmed (confirmed)';
$string['advancedfield_policyagreed'] = 'Site policy agreed (policyagreed)';
$string['advancedwarning'] = '"Advanced" fields below control login access directly, not ordinary profile data - used well (e.g. suspending an account when Monday marks someone as departed) they\'re powerful, but a bad or unexpected value here can genuinely lock someone out. For <strong>Authentication method</strong>, the Monday column\'s value must exactly match an auth plugin shortname enabled on this site (e.g. "manual", "nologin") - anything else effectively breaks that account\'s login. For <strong>Account suspended</strong>, <strong>Account confirmed</strong>, and <strong>Site policy agreed</strong>, map to a column whose value is exactly "0" or "1". For <strong>Username</strong>, a value that would duplicate a different account\'s existing username is refused automatically rather than applied - but a genuinely new, valid username is still applied as given, so a typo on Monday\'s side can still change how someone logs in. Double-check all of these before turning on a live sync.';
